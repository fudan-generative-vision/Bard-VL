...
class bard_vl_block_collate_fn:
    ...
    def find_response_span(self, input_ids):
        n = len(input_ids)
        header_prefix = self.assist_start_ids[:2]
        m = len(header_prefix)
    ...
    def __call__(self, batch):
        ...
        for messages in batch:
            ...
            response_ids = raw_ids[start_idx:end_idx]
            ...
            response_mask[len_pre + len_res_padded: len_pre + 2 * len_res_padded] = True
            ...
            # block padding区域
            noisy_valid_end = len_pre + len_res_padded + len(response_ids)
            # loss_mask[noisy_valid_end:] = False
            full_t[noisy_valid_end:] *= 2 # 降低padding token的loss权重, 避免padding占比过多，稀释真实梯度。
